...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	const buildWhere = <
		T extends { userId: typeof s.bands.userId; updatedAt?: typeof s.bands.updatedAt }
	>(
		table: T
	) => {
		if (lastSyncAt) {
	...
	const [
		...
		db
			...
			.where(eq(s.workoutTemplateExercises.userId, userId)), // Junction tables need full sync
