...
import { repairLocalCatalogDuplicates } from './catalogMerge';
...
export async function initDatabase() {
	...
	initPromise = (async () => {
		...
		await repairLocalCatalogDuplicates(db);
...
export async function liveQuery<
	Q extends { toSQL(): { sql: string; params: unknown[] } } & PromiseLike<unknown[]>
>(query: Q, callback: (rows: Awaited<Q>[number][]) => void): Promise<Awaited<Q>[number][]> {
	return new Promise((resolve) => {
		let resolved = false;
		const { sql, params } = query.toSQL();
		pglite.live.query(sql, params, (res: Results<Awaited<Q>[number]>) => {
			callback(res.rows);
			if (!resolved) {
				resolved = true;
				resolve(res.rows);
