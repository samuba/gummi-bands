<script lang="ts">
	...
	import type { PreviousExerciseData } from '$lib/stores/workout.svelte';
	...
	let notes = $state('');
	let previousData = $state<PreviousExerciseData>(null);
...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
	...
	<Dialog.Portal>
		...
		<Dialog.Content class="flex flex-col" interactOutsideBehavior="ignore">
			<Dialog.Title
				class="shrink-0 font-display text-xl font-semibold tracking-wide text-text-primary"
			>
				{exercise.name}
			</Dialog.Title>
							...
							<label for="full-reps" class="text-sm font-medium text-text-secondary"
								...
								{#if previousData && !currentLog}
									<span class="block text-xs text-text-muted"
										>last time: {previousData.fullReps}</span
									>
								{/if}
							</label>
						</div>
			...
			<button
				type="button"
				class="btn-primary mt-6 w-full shrink-0 rounded-lg py-3 font-semibold transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-50"
				disabled={!canSave}
				onclick={handleSave}
			>
